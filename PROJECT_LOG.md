...
## 六、桌面版与离线版（2026年9月16日）

- **exe 桌面版**：新增 `desktop/`（Electron 封装），`npm run dist` 一键打包 Setup + 便携版
- **单文件离线版**：`npm run build:offline` 生成单个 HTML（JS/CSS/知识库全部内联），双击即用
- **打包联动**：`desktop` 的 `npm run dist` 自动同步产出离线版 HTML 到 `离线版/`
- **存档持久化**：修复退出后角色卡丢失；固定端口 26571 + 单实例锁
- **5E/5R 双规则集**：基础规则可切换 2014(5E) / 2024(5R)
- **车卡流程对齐离线版**：技能熟练四分区选择、职业熟练自动写入
- **特性栏**：2024 特性真名 + 规则原文详情
- **法术描述**：自动生成并填入完整描述
- **规则速查**：角色卡页内嵌四页签检索库（规则检索 / 战斗轮讲解 / 小知识 / 换算查询）
- **范围格子小图**：法术距离旁显示格子示意，支持点击放置/瞄准，动态高亮覆盖格子与距离
